    ...
    public static void registerListeners() {
        LootTableEvents.MODIFY.register((lootTable, builder, lootTableSource, wrapperLookup) -> {
            ...
            addItemsToLootTable(builder, Blocks.BUDDING_AMETHYST.getLootTable().get(), lootTable,
                    new LootTableItem(ItemInit.BUDDING_AMETHYST_CRYSTAL, 1, 3));
            ...
            addItemsToLootTable(builder, BuiltInLootTables.ANCIENT_CITY, lootTable,
                    new LootTableItem(Blocks.DEEPSLATE_EMERALD_ORE, 1, 3, 0.075F),
                    new LootTableItem(ItemInit.SCULK_EMERALD, 1, 2, 0.12F));
            addItemsToLootTable(builder, BuiltInLootTables.END_CITY_TREASURE, lootTable,
                    new LootTableItem(ItemInit.END_DIAMOND, 1, 2, 0.075F),
                    new LootTableItem(ItemInit.END_EMERALD, 1, 4, 0.075F),
                    new LootTableItem(Items.DIAMOND_BLOCK, 0.12F));
            addItemsToLootTable(builder, BuiltInLootTables.BASTION_TREASURE, lootTable,
                    new LootTableItem(ItemInit.BLACKSTONE_EMERALD, 1, 2, 0.1F),
                    new LootTableItem(Items.NETHERITE_INGOT, 1, 4, 0.25F));
        ...
        LootTableEvents.REPLACE.register((key, original, source, registries) -> {
            ...
            if (Blocks.DEEPSLATE_EMERALD_ORE.getLootTable().get().equals(key)) {
                ...
                LootPool.Builder poolBuilder3 = LootPool.lootPool()
                        ...
                        .when(LootItemRandomChanceCondition.randomChance(ContextFloatProviders.between(0.25F, 0.125F)));
    ...
    public static void addItemsToLootTable(LootTable.Builder builder, ResourceKey<LootTable> target, ResourceKey<LootTable> lottTable, LootTableItem... entries) {
        if (target.equals(lottTable)) {
            for (LootTableItem entry : entries) {
                if (entry.count().left().isPresent())
                    addLootPool(builder, entry.item(), entry.count().left().get().getFirst(), entry.count().left().get().getSecond(), entry.chance());
                else
                    addLootPool(builder, entry.item(), entry.count().right().get(), entry.chance());
            }
        }
    }

    public static void addLootPool(LootTable.Builder builder, ItemLike item, int min, int max, float chance) {
        builder.pool(LootPool.lootPool()
                .setRolls(ContextIntProviders.between(min, max))
                .add(LootItem.lootTableItem(item))
                .when(LootItemRandomChanceCondition.randomChance(chance))
                .build());
    }

    public static void addLootPool(LootTable.Builder builder, ItemLike item, int exact, float chance) {
        builder.pool(LootPool.lootPool()
                .setRolls(ContextIntProviders.exactly(exact))
                .add(LootItem.lootTableItem(item))
                .when(LootItemRandomChanceCondition.randomChance(chance))
                .build());
    }

    public record LootTableItem(ItemLike item, Either<Pair<Integer, Integer>, Integer> count, float chance) {
        public LootTableItem(ItemLike item, float chance) {
            this(item, Either.right(1), chance);
        }
        public LootTableItem(ItemLike item, int exact) {
            this(item, Either.right(exact), 1.0F);
        }
        public LootTableItem(ItemLike item, int exact, float chance) {
            this(item, Either.right(exact), chance);
        }
        public LootTableItem(ItemLike item, int min, int max) {
            this(item, Either.left(new Pair<>(min, max)), 1.0F);
        }
        public LootTableItem(ItemLike item, int min, int max, float chance) {
            this(item, Either.left(new Pair<>(min, max)), chance);
        }
    }
}
